...
public class ShippingOrderAction extends _BaseStruts2Action implements ModelDriven<ShippingOrder> {
	...
	public String toedit(){
		...
		ShippingOrder obj = (ShippingOrder) oDao.get(ShippingOrder.class, model.getId());
//		ActionContext.getContext().getValueStack().push(obj);
	...
	public String save(){
		...
		PackingList packingList = (PackingList) pDao.get(PackingList.class, model.getId());
		...
		for(int i=0;i<exportIds.length;i++){
			...
			export.setState(3);			//0-草稿 1-已上报 2-装箱 3-委托 4-发票 5-财务
			oSet.add(export);
		}
